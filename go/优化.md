...
#### 再嵌入 4×4 AVX-512 micro-kernel（手写 Intrinsics）以及 Intrinsics和汇编有啥区别
...
|-------------|--------|------------|----------------|-----------|
| mm_ / mmintrin.h | MMX | 64 bit | _mm_add_pi16(a,b) | paddw mm0,mm1 |
...
| Intrinsic 名里是否带位宽 | 带：`_mm512_...` | SVE 不带：svadd_... | **不带位宽**，只标元素类型及 LMUL |
...
| **OpenBLAS** | 社区 + 各厂商贡献 | CPU (x86/ARM/POWER) | C / Fortran | ✅ BSD | 0.1-2 TFLOPS (64核) |
...
• **cuBLAS** kernel 内部是 **海量线程 + Tensor Core + warp shuffle**，并针对 Volta、Ampere、Hopper 架构有专门汇编或PTX。  
...
• **NCCL 管“GPU 数据”**，负责在 **MPI 已建好的进程之间** 做高效的 GPU-GPU 通信。 ----GPU间通信 
  典型流程：  
  ```
