...
### 买卖两只股票的最佳时机---动态规划
详细描述
两个相同长度的数组pricesA和pricesB它的第i个元素prices[i]表示对应给定股票第i天的价格。
有以下规定
1. 你只能在一只股票的交易完成后才能操作另一只股票（也就是你只在持有A股票的时候，不允许再买入B股票，只有等A股票卖出后，才允许买入B股票)
2. 对于一只股票而言，你只能选择某一天买入这只股票，并选择在未来的某一个不同的日子卖出该股票。
3. 同一只股票只允许交易一次，最终你必须完成两次交易。也就是说你一定要买入卖出A和B股票一次。
设计一个算法来计算你所能获取的最大利润，返回你可以从这笔交易中获取的最大利润。如果你不能获取任何利润或者无法满足上述条件，返回0。
```c++
#include <vector>
#include <algorithm>
#include <climits>
...
class Solution {
public:
    int maxProfit(vector<int>& pricesA, vector<int>& pricesB) {
        int n = pricesA.size();
        if (n < 2) return 0;

        // 1) 从左到右：以 i 结尾（卖出在 i 或之前）时，A 的最大利润
        vector<int> maxProfitA(n, 0);
        int minA = pricesA[0];
        for (int i = 1; i < n; ++i) {
            maxProfitA[i] = max(maxProfitA[i - 1], pricesA[i] - minA);
            minA = min(minA, pricesA[i]);
        }

        // 2) 从右到左：以 i 开头（买入在 i 或之后）时，B 的最大利润
        vector<int> maxProfitB(n, 0);
        int maxB = pricesB.back();
        for (int i = n - 2; i >= 0; --i) {
            maxProfitB[i] = max(maxProfitB[i + 1], maxB - pricesB[i]);
            maxB = max(maxB, pricesB[i]);
        }

        // 3) 枚举分界点 i：A 卖出 <= i，B 买入 >= i+1
        int ans = 0;
        for (int i = 0; i < n - 1; ++i) {
            ans = max(ans, maxProfitA[i] + maxProfitB[i + 1]);
        }
        // 如果最大利润 <= 0 则返回 0
        return max(ans, 0);
    }
...
```
- 只通过60%的测试用例，或许可以参考[小红书](https://www.xiaohongshu.com/login?redirectPath=https%3A%2F%2Fwww.xiaohongshu.com%2Fexplore%2F68aed507000000001b01c558%3Fapp_platform%3Dandroid%26ignoreEngage%3Dtrue%26app_version%3D8.93.0%26share_from_user_hidden%3Dtrue%26xsec_source%3Dapp_share%26type%3Dnormal%26xsec_token%3DCBhenHTfY79W4K4_5nCA_hvgt_qcCDIdKgJqUn9qDJFBk%3D%26author_share%3D1%26xhsshare%3DWeixinSession%26shareRedId%3DODk4ODg3Nzs2NzUyOTgwNjdJOThGPUdA%26apptime%3D1756561851%26share_id%3Dc170e7c39c44482ba7466ca1e21ca91a%26share_channel%3Dwechat)

### 最长回文子串查找---动态规划
- 如果最大回文长度相等，则返回第一个出现的最大回文子串
```c++
class Solution {
public:
    string longestPalindrome(string s) {
        ...
        vector<vector<char>> dp(n, vector<char>(n, 0));   // char 足够，省内存
...
### 互斥字符串---动态规划
互斥字符串:一个字符串仅由"A"“B"两个字符组成，且字符串内任意相邻两个字母不相同
可以将A变成B或者将B变成A，从而变成互斥字符串，变成互斥字符串，最少需要多少步骤？
...
public:
    int minSteps(string s) {
        ...
        // 计算变成两种交替模式所需的步数
        int toPattern1 = 0; // ABAB...模式
        int toPattern2 = 0; // BABA...模式
        
        for (int i = 0; i < n; i++) {
            // 模式1：偶数位置是A，奇数位置是B
            char expected1 = (i % 2 == 0) ? 'A' : 'B';
            if (s[i] != expected1) {
                toPattern1++;
            }
            
            // 模式2：偶数位置是B，奇数位置是A
            char expected2 = (i % 2 == 0) ? 'B' : 'A';
            if (s[i] != expected2) {
                toPattern2++;
            }
        }
        
        return min(toPattern1, toPattern2);
    }
};
```
### 一些选择
1. 网络
- 交换机识别MAC地址
- 路由器识别IP地址并根据路由表选择下一跳
- 网关识别IP地址并根据路由表选择下一个网关
2. 数据库
- 视图是基于真实数据表建立的，但是视图是虚拟表，不存储实际数据，在视图上再建立一个视图，是可以的。
3. 算法
- 快速排序：时间复杂度为O(nlogn)
- 堆排序：分为俩个步骤
    - 建堆：从最后一个非叶子节点开始，从下往上，对每个节点进行堆调整，下层的节点（靠近叶子）只需要进行少量的比较和交换，上层的节点需要更多操作，所以求和平均，所有节点下沉操作（即建堆过程）的总时间复杂度为O(N)。而不是因为每个元素的下沉操作最坏情况下需要O(log n) 的时间，而总共n个元素，所以错误的认为总时间复杂度为O(nlogn)
    - n - 1次堆调整：重复的将堆顶元素（最大值或者最小值）取出与堆底元素交换，然后重新调整堆，直到所有元素都被取出，时间复杂度为O(n - 1) * O(logn) = O(nlogn)
4. 进程线程
- 每段程序必然有一个进程，一个进程必然有一个主线程
- 进程是独立的可执行实体，线程依赖进程，不独立
5. 数据结构
- 哈希表：插入，删除，查找都是O(1)
- 栈和队列：压栈和弹栈是O(1)，查找O(N)
- 二叉搜索树：插入，删除，查找都是O(logn)，在最坏情况下，比如：树变成一条链，时间复杂度是O(N)
